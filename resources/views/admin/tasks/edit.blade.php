        ...
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('milestone') ? 'is-invalid' : '' }}" name="milestone_id" id="milestone_id">
                    @foreach($milestones as $id => $entry)
                        <option value="{{ $id }}" {{ (old('milestone_id') ? old('milestone_id') : $task->milestone->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('milestone'))
                    <span class="text-danger">{{ $errors->first('milestone') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.milestone_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="hours_locked">{{ trans('cruds.task.fields.hours_locked') }}</label>
                <input class="form-control {{ $errors->has('hours_locked') ? 'is-invalid' : '' }}" type="number" name="hours_locked" id="hours_locked" value="{{ old('hours_locked', $task->hours_locked) }}" step="0.01">
                @if($errors->has('hours_locked'))
                    <span class="text-danger">{{ $errors->first('hours_locked') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.hours_locked_helper') }}</span>
            </div>
            <div class="form-group">
