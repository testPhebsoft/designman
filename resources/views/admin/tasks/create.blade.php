    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('milestone') ? 'is-invalid' : '' }}" name="milestone_id" id="milestone_id">
                    @foreach($milestones as $id => $entry)
                        <option value="{{ $id }}" {{ old('milestone_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hours_locked') ? 'is-invalid' : '' }}" type="number" name="hours_locked" id="hours_locked" value="{{ old('hours_locked', '') }}" step="0.01">
